            ...
            with patch('socket.gethostbyname', return_value="127.0.0.1"):
                with requests_mock.Mocker() as m:
                    ...
                    if line_items_exist:
                        ...
                        m.post(line_items_url, text=json.dumps({
                            'scoreMaximum': 100.0,
                            'tag': 'score',
                            'id': 'http://canvas.docker/api/lti/courses/1/line_items/1',
                            'label': 'Score'
                    ...
                    m.get('http://canvas.docker/api/lti/courses/1/line_items/1/results',
                          text=json.dumps([{
                              'resultScore': 13.0,
                              'resultMaximum': 100.0,
    ...
    def test_send_scores(self):
        ...
        with patch.object(DjangoMessageLaunch, "_get_jwt_body", autospec=True) as get_jwt_body:
            ...
            with patch('socket.gethostbyname', return_value="127.0.0.1"):
                with requests_mock.Mocker() as m:
                    m.post(self._get_auth_token_url(), text=json.dumps(self._get_auth_token_response()))
                    m.get('http://canvas.docker/api/lti/courses/1/line_items',
                          text=json.dumps([{
                              'scoreMaximum': 100.0,
                              'tag': 'score',
                              'id': 'http://canvas.docker/api/lti/courses/1/line_items/1',
                    ...
                    m.post('http://canvas.docker/api/lti/courses/1/line_items/1/scores',
